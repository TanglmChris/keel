...
- **Writing the marker's name inside a Findings block counts as using it.** A finding that *discusses* the disposition — quoting `Resolved here:` while explaining something about it — is refused, because the next token is prose rather than evidence. Measured on this change's own task 1.3. Nothing distinguishes a mention from a use in free prose, and erring the other way would let any block escape the check by embedding the marker in a sentence. The author's fix is to name the disposition in words instead of quoting it, which is one edit; the alternative is teaching the gate to parse Markdown, which is the same prose problem as below and a worse trade.
