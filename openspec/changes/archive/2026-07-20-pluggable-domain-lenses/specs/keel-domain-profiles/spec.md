...
**Reason**: Replaced by "Keel supports pluggable domain lenses" — domain knowledge is no longer bundled inside `keel-align-expectations`; it is user-authored in `keel/lenses/`.

### Requirement: Domain references serve execution and review phases

**Reason**: Replaced by "Domain lenses serve execution and review phases", which consults `keel/lenses/` instead of the bundled `references/`.

### Requirement: Domain reference authority stays single-source

**Reason**: Replaced by "Shipped lens templates stay single-source" — single-source now governs `assets/lenses/` templates, and explicitly does not police user lenses.

### Requirement: Keel diagnoses domain profile surface

**Reason**: Replaced by "Keel diagnoses the domain lens surface".
