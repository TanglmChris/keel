<!-- Durable record for the modernize-lens-vocabulary change. Per design.md F5
     the capability rename and Purpose rewrite are direct live edits promoted with
     `openspec archive --skip-specs`; this delta is the format-valid record of the
     renamed capability's requirement set (reject scenario moved off "are
     bundled"). The generic-vocabulary sweep of the other three capabilities is
     recorded in design.md D3, not as separate delta files, because it is
     vocabulary-only and no delta is applied under --skip-specs. -->
