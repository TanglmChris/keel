...
A change-level section of tasks.md — `## Invalidates`, `## Expectation Coverage` — MUST end at the next `##` heading or at the next task, whichever comes first. This is the mirror of the boundary applied to a task's own body, and both MUST be computed by one shared rule rather than by two that agree today. A tasks file's dominant structure is a list, so a section bounded only by the next heading extends over the task list whenever it is not the file's last section.
...
A line inside a task body MUST NOT close, satisfy, or contribute an entry to a change-level section. In particular a task field entry of `none` MUST NOT be read as the section's `- None.`, and an `E<n>` or `I<n>` line a task declares under `Covers` MUST NOT be read as a section entry.
...
- **THEN** `change-close` returns the same verdict as it does for the identical section in the file's tail
...
- **WHEN** a change-level section sits above the task list, declares an entry with no closure, and a task body declares a field entry of `none`
