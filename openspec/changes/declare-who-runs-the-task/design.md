...
**D6** — The write-capable brief is a **separate contract beside** the helper brief; `helper.js` is untouched. *Basis:* F5. Byte identity is the only thing the helper contract proves, and admitting a mutation verb would destroy it. Two contracts with one honest guarantee each is better than one contract with a guarantee that holds sometimes.
