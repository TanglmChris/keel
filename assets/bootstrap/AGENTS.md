...
- One current agent holds write authority; helpers return read-only report/evidence, and a declared delegate (`delegation:` in `keel/config.yaml`, with an active guard) may write inside Touch while the current agent re-runs every check before recording Evidence. No commit, sync, or archive without explicit authorization.
