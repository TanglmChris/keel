# Keel v5.2.3 Agent Protocol

<!-- keel:start version=5.2.3 -->
## Session Start
...
- If this repo is missing or partially missing the v5.2.3 protocol, prompt before install and suggest keel --init --target <target>.
<!-- keel:end -->
...
- **Recording a finding on a task**: cite the issue URL directly in the Review `Findings` line — `keel gate task-complete` accepts an absolute `https://…` reference as a durable owner, alongside a `Discard reason:`/`Discard rationale:` prefix, a `keel/archive/…` path, and an existing `openspec/changes/…` artifact. The same forms apply to an `## Expectation Coverage` `Durable owner:` entry. `keel/HANDOFF.md` is still refused. A `keel/archive/follow-ups/` note is now optional — write one only when the finding needs evidence the issue body does not carry. Notes written before this (issue #12, fixed in 5.2.4) record the old workaround and are kept as historical evidence.
